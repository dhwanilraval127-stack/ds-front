/**
 * Universal Camera Component for All Features
 * Supports image capture, live analysis, and voice control
 */
...
const UniversalCamera = ({ 
  ...
  isAnalyzing = false,
  analysisResult = null,
  featureType = 'general', // 'plant-disease', 'soil', 'ndvi', etc.
  ...
  useEffect(() => {
    if (voiceSupported && showVoiceControl) {
      const timer = setTimeout(() => {
        startListening();
        speak(language === 'hi' 
          ? `${getFeatureName()} कैमरा तैयार है। धरती बोलकर मुझे सक्रिय करें।` 
          : `${getFeatureName()} camera ready. Say Dharti to activate me.`
        );
      }, 1000);
      return () => clearTimeout(timer);
    }
  }, [voiceSupported, showVoiceControl]);

  // Get feature name for voice prompts
  const getFeatureName = () => {
    const names = {
      'plant-disease': language === 'hi' ? 'पौधा स्कैनर' : 'Plant Scanner',
      'soil': language === 'hi' ? 'मिट्टी स्कैनर' : 'Soil Scanner',
      'ndvi': language === 'hi' ? 'एनडीवीआई स्कैनर' : 'NDVI Scanner',
      'general': language === 'hi' ? 'कैमरा' : 'Camera'
    };
    return names[featureType] || names.general;
  };
  ...
  const handleVoiceCommand = useCallback((result) => {
    switch (result.action) {
      ...
      case 'analyze':
        ...
        break;
      case 'detect_disease':
        if (featureType === 'plant-disease') captureAndAnalyze();
        break;
      case 'detect_soil':
        if (featureType === 'soil') captureAndAnalyze();
        break;
      case 'stop':
        onClose();
        break;
      ...
      default:
        ...
        speak(language === 'hi' 
          ? 'कृपया कहें: फोटो खींचो, विश्लेषण करो, या रुको' 
          : 'Please say: capture photo, analyze, or stop'
        );
        break;
    }
  }, [capturedImage, featureType, language]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-20 p-4 flex items-center justify-between bg-gradient-to-b from-black/60 to-transparent">
        ...
        <div className="text-white text-center">
          ...
          {isListening && (
            ...
            >
              <FiMic className={`w-3 h-3 ${isActivated ? 'text-green-400' : 'text-white'}`} />
              {isActivated 
      ...
      <div className="bg-black/80 px-6 py-6 pb-8 safe-bottom">
        ...
        {/* Voice hint */}
        {voiceSupported && showVoiceControl && !capturedImage && (
...
// Helper function for guide text
const getGuideText = () => {
  // This would be implemented based on featureType
